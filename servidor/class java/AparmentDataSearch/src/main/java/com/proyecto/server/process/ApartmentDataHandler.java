	...
	public void process(Exchange exchange) throws Exception {
		...
		if(bandera==0) {
			...
			for(int i=0; i<count.getCount();i++) {
				...
				inmu.setNombre((String) map.get(i).get(0));
				inmu.setPrecio((String) map.get(i).get(1));
				inmu.setTipo((String) map.get(i).get(2));
				inmu.setEstado_inmueble((String) map.get(i).get(3));
				inmu.setEstado((String) map.get(i).get(4));
				inmu.setBarrio((String) map.get(i).get(5));
				inmueble.add(inmu);
			}
